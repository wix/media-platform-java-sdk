...
import static com.wix.mediaplatform.image.Metadata.KEY_MIME_TYPE;
import static com.wix.mediaplatform.image.StringToken.*;
import static com.wix.mediaplatform.image.framing.Crop.*;
            ...
            switch (method.getName()) {
                case "crop":
                    method.invoke(
                            ...
                            parseInt(params.get(KEY_X)[0]),
                            parseInt(params.get(KEY_Y)[0]),
                            parseFloat(params.get(KEY_SCALE)[0])
                    );
                    params.remove(KEY_WIDTH);
                    params.remove(KEY_HEIGHT);
                    params.remove(KEY_X);
                    params.remove(KEY_Y);
                    params.remove(KEY_SCALE);
                    break;
                default:
